...
const registerDepense=async (req,res,next)=>{
    ...
    res.render("depenses/register_depense",{user:res.locals.user,typeDepense});
}

const getDepenses = async (req,res,next)=>{
    const depense= await Depense.findAll();
    
    res.render("depenses/table_depenses",{depense,user:res.locals.user});
}

const postDepense = async (req,res,next)=>{
    const utilisateur = res.locals.user.nom_user+""+res.locals.user.prenom_user;
...
const editDepenses = async (req,res,next)=>{
    ...
    res.render("depenses/edit_depense",{depense,user:res.locals.user}); 
}

const typeDepenses = async (req,res,next)=>{
    res.render("depenses/type_depense",{user:res.locals.user}); 
}
